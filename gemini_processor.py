#!/usr/bin/env python3
"""
Gemini Image Processor for MathRush DataProcessor.
Processes manually segmented problem images and updates database with Gemini-extracted content.
"""
...
try:
    from processors import DatabaseSaver, GeminiExtractor
...
class GeminiProcessor:
    """Gemini processor for manually segmented problem images."""
    ...
    def process_problem_images(self, problems: List[Dict], exam_metadata: Dict) -> Tuple[List[Dict], List[Dict]]:
        """
        Process problem images through Gemini extraction and create database records.
        ...
        for problem in problems:
            ...
            try:
                ...
                # Extract content using Gemini extractor
                extracted_data = self.gemini_extractor.extract_from_image(
                    problem_image, 
                ...
                if extracted_data.get('extraction_error', False):
                    raise Exception(f"Gemini extraction failed: {extracted_data.get('content', 'Unknown error')}")
                
                # Create new record with Gemini content (no manual answers yet)
                problem_record = {
                    ...
                    'source_info': {
                        ...
                        'gemini_processed_timestamp': time.strftime('%Y-%m-%dT%H:%M:%S')
                    },
    ...
    def update_failed_records(self, processed_problems: List[Dict]) -> Dict[str, int]:
        """
        Update existing database records with new Gemini content.
    ...
    def process_exam_directory(self, exam_dir: str, retry_failed: bool = False) -> bool:
        """
        Process entire exam directory and upsert database records with Gemini content.
        ...
        try:
            ...
            # Step 3: Process images through Gemini
            print("\n🤖 STEP 3: Processing images through Gemini...")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Gemini Image Processor for MathRush DataProcessor - Extracts content from images and creates database records",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Process specific exam directory (upserts records with Gemini content)
  ...
  python gemini_processor.py input/2020-12-03_suneung_가형/ --retry-failed
  
  # Process with verbose logging
  python gemini_processor.py input/2020-12-03_suneung_가형/ --verbose
  
  # Process multiple exams in input directory
  python gemini_processor.py input/ --recursive

Workflow:
  1. Run gemini_processor.py to extract content and upsert records (creates or updates automatically)
  2. Run manual_answer_input.py to add answers to existing records
  3. Re-run gemini_processor.py anytime to update existing records with new Gemini extractions
"""
    )
